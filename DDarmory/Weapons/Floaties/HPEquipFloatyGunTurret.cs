...
public class HPEquipFloatyGunTurret : HPEquipGunTurret, IMassObject
{
    ...
    public void GrabComponents()
    {
        ...
        var flag2 = usingWheels;
        if (flag2) StartWheels();
    }

    private void FixedUpdate()
    {
        var flag = _deployed || alwaysDeployed;
        if (flag)
        {
            Debug.Log("Steering..");
            Steer(_vehicleInputManager.outputPYR);
            var traverse = Traverse.Create(_vehicleInputManager);
            var wheelBrakesBound = _vehicleInputManager.wheelBrakesBound;
            if (wheelBrakesBound)
            {
    ...
    [Range(0f, 1f)] public float maxBrakeForce;

    // Token: 0x0400002B RID: 43
    [Range(0f, 1f)] public float steeringForce;
