...
public class LoiterGuidance : MissileGuidanceUnit
{
    ...
    {
        var opticalTargetFromView = TargetManager.instance.GetOpticalTargetFromView(missile.actor, visualRange,
            Actor.GetRoleMask(roleMask), 300f, eyeTf.position, eyeTf.forward, fov);
        var flag = opticalTargetFromView && opticalTargetFromView.alive &&
                   !opticalTargetFromView.GetComponent<imtargetedlmao>();
        Vector3 result;
        if (flag)
        {
    ...
    public UnityEvent OnTargetAcquired = new();
